...
我用 AI 编程代理阅读代码、追踪数据流、运行检查、实际测试 MCP 协议、对照源码核查已有调查，并把结果整理成 `research.md` 和 `tasks.md`。这一阶段主要用它来调查和验证，还没有让它修改业务代码。
...
我也核对了 Node 的 `exec` 行为。它确实有默认输出缓冲区，但项目自己仍然没有明确的超时、取消、截断和命令执行规则。
...
- 使用 `repo_path` 调用 MCP `review_repository` — 重现路径被忽略的问题。
- `npm run inspector -- review --repo . --base-ref main --validate false` — 重现验证失败直接中止，退出码为 1。
...
- MCP 路径问题已经确认，但源码还没修。
- 验证命令仍然通过 Shell 执行，没有项目级超时和安全规则。
- 测试还没有覆盖 MCP 调用、Git 边界情况、失败检查和超大输出。

接下来我会做三件事：

1. 修好 MCP 的输入和错误处理，再补一个隔离的 JSON-RPC 测试。
2. 把失败检查变成有结构、有上限、看得见的结果，并写清楚信任边界。
3. 重新跑 typecheck、测试、build 和 MCP 集成测试，再根据最终改动和准确时间更新本文档。

## 大致集中工作时间

- 开始（Start）：2026-07-31（调查检查点；最终提交前确认准确时间）
- 结束（Finish）：待实现和最终验证完成
